...
void main() {
    ...
    v_sphere = n;
    {
        ...
        if (kb < 0.5) { lsum += unpackUnorm4x8(cy.color_rgba8).rgb; lnum += 1.0; }
        if (kc < 0.5) { lsum += unpackUnorm4x8(cz.color_rgba8).rgb; lnum += 1.0; }
        v_land_color = lnum > 0.0 ? lsum / lnum : unpackUnorm4x8(cd.color_rgba8).rgb;
